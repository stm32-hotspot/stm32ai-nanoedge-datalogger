...
#endif
#ifndef SAMPLES
  #define SAMPLES                       128                     /* Should be between 16 & 256 */
#endif
...
static void print_float(float float_input);
      ...
      if (wtm_flag) {
        ...
        if (data_left == 0) {
          ...
          if (neai_cnt < NEAI_LEARN_NB) {
            ...
            printf("Learn: %d / %d. NEAI learn return: %d.\n", neai_cnt, NEAI_LEARN_NB, neai_state);
          }
          else {
            neai_state = neai_anomalydetection_detect(neai_buffer, &neai_similarity);
            printf("Similarity: %d / 100. NEAI detect return: %d.\n", neai_similarity, neai_state);
          }
#else
          for (uint16_t i = 0; i < AXIS * SAMPLES; i++) {
            print_float(neai_buffer[i]);
          }
          printf("\n");
#endif
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin : LSM6DSO_INT_Pin */
  GPIO_InitStruct.Pin = LSM6DSO_INT_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(LSM6DSO_INT_GPIO_Port, &GPIO_InitStruct);
...
static void lsm6dso_get_buffer_from_fifo(uint16_t nb)
{
  ...
  for (uint16_t i = 0; i < nb; i++) {
    ...
    if(reg_tag == LSM6DSO_XL_NC_TAG) {
      neai_buffer[(AXIS * neai_buffer_ptr) + (AXIS * i)] = lsm6dso_convert_accel_data_to_mg((uint16_t) buff_tmp[(FIFO_WORD * i) + 2] << 8 | buff_tmp[(FIFO_WORD * i) + 1]);
      neai_buffer[(AXIS * neai_buffer_ptr) + (AXIS * i) + 1] = lsm6dso_convert_accel_data_to_mg((uint16_t) buff_tmp[(FIFO_WORD * i) + 4] << 8 | buff_tmp[(FIFO_WORD * i) + 3]);
      neai_buffer[(AXIS * neai_buffer_ptr) + (AXIS * i) + 2] = lsm6dso_convert_accel_data_to_mg((uint16_t) buff_tmp[(FIFO_WORD * i) + 6] << 8 | buff_tmp[(FIFO_WORD * i) + 5]);
    }
    else if(reg_tag == LSM6DSO_GYRO_NC_TAG) {
      neai_buffer[(AXIS * neai_buffer_ptr) + (AXIS * i)] = lsm6dso_convert_gyro_data_to_mdps((uint16_t) buff_tmp[(FIFO_WORD * i) + 2] << 8 | buff_tmp[(FIFO_WORD * i) + 1]);
      neai_buffer[(AXIS * neai_buffer_ptr) + (AXIS * i) + 1] = lsm6dso_convert_gyro_data_to_mdps((uint16_t) buff_tmp[(FIFO_WORD * i) + 4] << 8 | buff_tmp[(FIFO_WORD * i) + 3]);
      neai_buffer[(AXIS * neai_buffer_ptr) + (AXIS * i) + 2] = lsm6dso_convert_gyro_data_to_mdps((uint16_t) buff_tmp[(FIFO_WORD * i) + 6] << 8 | buff_tmp[(FIFO_WORD * i) + 5]);
    }
  }
  neai_buffer_ptr += nb;
...
/*
 * @brief  Print float data without using newlib-nano in order to save memory
 *
 * @param  float_input: the data to be print
 *
 * @return No
 *
 */
void print_float(float float_input)
{
  uint8_t char_nb = 0;
  char float_string[20];
  int32_t integer_res = 0;

  /* If  input is negative */
  if (float_input < 0.0) {
      float_input = -1.0 * float_input;
      putchar('-');
  }

  integer_res = (int32_t) (1000 * float_input);
  char_nb = snprintf(float_string, 20, "%ld", integer_res);
  ...
  if (float_input == 0.0) {
    ...
    putchar('0');
    putchar('.');
    for (uint8_t i = 0; i < 3 - char_nb; i++) {
      putchar('0');
    }
    for (uint8_t i = 0; i < char_nb; i++) {
      putchar(float_string[i]);
    }
  }
  else {
    for (uint8_t i = 0; i < char_nb - 3; i++) {
      putchar(float_string[i]);
    }
    putchar('.');
    for (uint8_t i = char_nb - 3; i < char_nb; i++) {
      putchar(float_string[i]);
    }
  }
  putchar(' ');
